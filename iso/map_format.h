...
#pragma pack(pop)
    ...
    inline bool save_isomap(const ChunkMap& map, const std::string& filepath) 
    {
        ...
        header.entity_count = (uint32_t)std::count_if(ents.begin(), ents.end(),
        ...
        for (const auto& e : ents) {
            // Only save static and dynamic entities (not networked)
            if (e.is_networked()) continue;
            ...
            rec.flags = static_cast<uint8_t>(e.flags);
    ...
    inline bool load_isomap(ChunkMap& map, const std::string& filepath) {
        ...
        for (uint32_t i = 0; i < header.entity_count; ++i) {
            ...
            e.type = static_cast<EntityType>(rec.type);
            e.flags = static_cast<EntityFlags>(rec.flags);
        ...
        if (ent_file.is_open()) {
            ...
            if (std::memcmp(ent_magic, "ISOE", 4) == 0) {
                ...
                for (uint32_t i = 0; i < ent_count; ++i) {
                    ...
                    e.flags = static_cast<iso::EntityFlags>(flags);
